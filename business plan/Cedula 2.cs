            ...
            else
            {
                ...
                for (int i = 0; i <= dgvCed2.Rows.Count - 1; i++)
                {
                    if (dgvCed2.Rows[i].Cells[3].Value != null)
                    {
                        ...
                        query = "INSERT INTO  cedula2 (nombre,categoria,Pasignacion,AUnid,AImpo) VALUES('" + tbNombre.Text + "','" + categoria + "'," + porcientoAsig.ToString() + "," + AUnid.ToString() + "," + AImpo.ToString() + ");";
                        cmd = new MySqlCommand(query, Conn);
                        cmd.ExecuteNonQuery();
                    }
                }
